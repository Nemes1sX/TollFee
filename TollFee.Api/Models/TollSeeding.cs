...
namespace TollFee.Api.Models
{
    public class TollSeeding
    {
        ...
        public async void SeedData(int year)
        {
            var seedingYearExists = await _context.TollFrees.Where(x => x.Year == year).ToListAsync();
            ...
            await _context.TollFrees.AddRangeAsync(holidayYearList);
